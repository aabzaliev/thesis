...
data <- read.table(paste("./data/", "portfolio.txt", sep=""),
                     header=TRUE, sep=",")
data <- list(data, as.integer(N), as.double(R),
              as.double(Rmax))
...
returns <- list(ret, names(data[[1]]), data[[2]],
              data[[3]], data[[4]])
  ...
  R <- returns[[4]]; 
  Rmax <- returns[[5]];
  #here we have an option - how to solve this stuff
  #if we add lambda and identity matrix then we have l2 regularization
  lambda <- 0.001
  lambda_matr <- diag(lambda, J, J)
  S <- cov(returns[[1]]);
  #AA this is a new line 
  #S <- S + lambda_matr
  Q <- solve(S); 
  u <- rep(1,J);
  ...
  z <- (r - R)/cml[1]; f <- Q%*%(p - R*u)/(cml[1]*cml[1]);
  wcml <- matrix(rep(0,J*M), nrow=J); wf <- rep(0,M);
  for(m in 1:M){
    wcml[,m] <- (r[m] - R)*f; 
    wf[m] <- 1 - wcml[,m]%*%u;
  }
  wcml <- rbind(wcml, t(wf))
  #mp is a variance and return value for market portfolio 
  mp <- c(cml[1]/(a[1,2] - a[1,1]*R),
          (a[2,2] - a[1,2]*R)/(a[1,2] - a[1,1]*R));
  #weights of the market portfolio
  wmp <- Q%*%(p - R*u)/(a[1,2] - a[1,1]*R);
  minp <- c(sqrt(1/a[1,1]), cml[1]*sqrt(1/a[1,1]) + R);
  wminp <- (minp[2] - R)*f; wfminp <- 1- t(wminp)%*%u;
  wminp <- rbind(wminp, wfminp)
  
  results <- list(s, z, r, ss, p, cml, wcml, mp, wmp,
              minp, wminp)
  
  #checking the analytical solution from the paper:
  
  excess_returns <- returns[[1]] - R
  p_of_excess_returns <- colMeans(excess_returns)
  
  muu <- (t(p_of_excess_returns)%*%Q%*%p_of_excess_returns)/(t(u)%*%Q%*%p_of_excess_returns)
  
  #this is exactly as wmp variable
  tocheck_weights <- (p_of_excess_returns%*%Q)/(as.double(t(u)%*%Q%*%p_of_excess_returns))
  
  
  dat <- log(data[[1]]); M <- nrow(dat); J <- ncol(dat)
  
  ymax = max(dat); ymin = min(dat); mycolors <- rainbow(J);
  s <- results[[1]]; z <- results[[2]]; r <- results[[3]];
  ss <- results[[4]]; p <- results[[5]];
  cml <- results[[6]];
  mp <- results[[8]]; wmp <- results[[9]];
  minp <- results[[10]]; wminp <- results[[11]];
  #postscript(file="./results2/fig1.eps", onefile=FALSE,
  #           horizontal=FALSE, height=10, width=5);
  jpeg('Assets_into_ridge.jpg')
  par(mfrow=c(2,1));
  id <- c(1:nrow(dat));
  ...
  legend("topleft", names(dat), cex=0.5, pch=rep(15, J),
         col=mycolors);
  ret <- returns[[1]]; ymax = max(ret); ymin = min(ret);
  id <- c(1:nrow(ret));
  ...
  plot(s,r,xlim=c(0, max(s)),ylim=c(min(r, p),max(r, p)),
       ...
       main = "Capital Market Line, MVP2, MP");
  ...
  abline(h=0, lty=2); abline(v=mp[1], lty=3);
  text(mp[1], min(f), pos=4, cex=0.5, "MP");
  abline(v=minp[1], lty=3);
  text(minp[1], min(f), pos=4, cex=0.5, "MVP2");
  legend("topleft", c(names(p), "RFA"),
         cex=0.5, pch=rep(15, J+1), col=mycolors);
  dev.off()
  #postscript(file="./results2/fig3.eps", onefile=FALSE,
  #           horizontal=FALSE, height=10, width=5);
  jpeg('2barplots_ridge.jpg')
  par(mfrow=c(2,1));
  barplot(wminp, main="Minimum Variance Portfolio 2",
